...
![[Screenshot 2026-03-03 at 9.24.34 PM 1.png]]

- **Fecal** samples show the **richest/most complex** community profiles (many taxa contributing across samples).
    
- **Skin** samples are often dominated by a **few taxa** (large blocks), indicating a more strongly dominated community structure.
    
- **Udder** samples are **intermediate**—less complex than fecal but not as dominated as some skin samples.
    
- **Nasal** and **oral** samples tend to show **lower richness** and more moderate dominance patterns than fecal.
    
- **Controls (including EC/positive controls)** appear **compositionally distinct** from the true sample groups.
...
A prominent shared signal between **udder and skin** corresponds to a **Staphylococcales / Staphylococcaceae** lineage (a common “skin-associated” dominant group visible in both body sites).

**_Question 4**: Which samples (still sorted by body_site) have higher alpha diversity in terms of observed features?
**Fecal and skin** have the **highest observed features (richness)**, **udder** is intermediate, and **nasal/oral** are lower; **controls** are the lowest.  
Observed features differ **significantly** by body_site (**Kruskal–Wallis p ≈ 3.0 × 10⁻²⁰**).

**Question 5**: do all samples contain archaea as well?
**No.** In the filtered taxa bar plot (Level 7), the displayed taxa are overwhelmingly **Bacteria** (legend entries show `d__Bacteria...`), so archaea are **not consistently evident** across samples in this filtered output.

**No — not all samples contain detectable archaea.** Archaea are present only at **very low abundance** and appear in **some** samples (most reads are bacterial).

**Question 6**: why do we filter out sp004296775?
We filter out **sp004296775** because the workflow instructs us to remove it as an **unwanted/non-target taxon/feature**, along with **mitochondria** and **chloroplast**, so it does not **bias** the taxa bar plot and downstream comparisons. In my results, **sp004296775 does not appear in the exported taxonomy/legend after filtering**, confirming it was removed from the filtered table used for the bar plot.

**Question 7**: what is the difference between these two flags? 
`--p-exclude mitochondria,chloroplast,sp004296775` vs `--p-include ...`

- `--p-exclude ...` **removes** the listed taxa/features from the output.
    
- `--p-include ...` **keeps only** what matches the include rule and filters everything else out.

**Question 8**: do the positive controls look the same as each other? Yes or No?
**No.** The positive controls show **different dominant taxa patterns**, so they are not identical.

**Question 9**: Do the negative/extraction controls (Samples labeled as EC), look like the positive controls? Yes or no? 
**No.** EC controls do **not** resemble the positive controls in composition.

**Question 10**: do the negative/extraction controls (Samples labeled as EC), look like the real samples? Yes or no?
**No.** EC controls appear **compositionally distinct** from the real sample groups (fecal/udder/nasal/oral/skin).
...
../dada2/cow_seqs_dada2_filtered300.qza \  
...
Job ID: 24442582  
Cluster: alpine  
User/Group: c837277701@colostate.edu/c837277701pgrp@colostate.edu  
State: COMPLETED (exit code 0)  
Nodes: 1  
Cores per node: 8  
CPU Utilized: 03:18:08  
CPU Efficiency: 12.39% of 1-02:38:40 core-walltime  
Job Wall-clock time: 03:19:50  
Memory Utilized: 8.48 GB  
Memory Efficiency: 28.27% of 30.00 GB (30.00 GB/node)
