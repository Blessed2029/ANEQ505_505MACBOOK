...
One way to get data from QIIME2 into a format that can be used in R is to export or tabulate the QIIME2 results as text-based files such as `.tsv` or `.txt`, and then import those files into R using functions like `read_tsv()`.
...
The **fecal** body site appeared most distinct in the taxa bar plot.
It looks different because its microbial composition is dominated by taxa that are much more abundant in fecal samples than in the other body sites. In contrast, sites like skin, nasal, oral, and udder tend to share more similar surface-associated communities. Because the gut environment is very different from external body surfaces, fecal samples usually contain a more specialized and distinct microbial community.
...
I chose `5000` for `--p-min-frequency`. This should match the core metrics parameter `--p-sampling-depth`, which was also `5000`. These values need to be the same so that the same sequencing depth cutoff is used across analyses, helping keep the samples included in ANCOM-BC2 consistent with those used in the diversity analyses.

4. Why do we filter out samples with low frequency and low abundance ASVs?
We filter out samples with low frequency and low-abundance ASVs to reduce noise and improve the reliability of the analysis. Low-frequency samples may have too few reads to represent the microbial community well, and very rare ASVs may reflect sequencing errors, contaminants, or features that are too sparse for stable statistical testing. Removing them helps make the ANCOM-BC2 results more robust and easier to interpret.
...
-the **most enriched genus in skin compared to fecal** was **Atopostipes**
-the **most depleted genus in skin compared to fecal** was **Streptococcus**
> When comparing skin to fecal at adjusted p < 0.05, the most enriched genus in skin was **Atopostipes**, while the most depleted genus in skin was **Streptococcus**.
...
--m-metadata-file ../ancombc2/cow_metadata_nocontrols.txt \  
...
### **Questions:**
1. Why might removing controls be important before downstream analysis? Removing controls is important because controls do not represent true biological cow body-site communities. If they are included, they can introduce contamination signals or technical noise and make downstream analyses less accurate.
2. what 2 features that are high in fecal samples?   Alistipes and **Bacteroides**
3. what are 2 features that are low in nasal?  **Succinivibrio** and **Alistipes**
4. what is the accuracy of your model, and if the accuracy of the classifier is high, what does that suggest about the microbial compositions of each site? 
   The model accuracy was **0.823529** (about **82.35%**). A high classifier accuracy suggests that the microbial compositions of the different body sites are distinct enough for the model to classify them fairly well.